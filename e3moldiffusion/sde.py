...
def get_ddpm_params(beta_min: float = 0.1, beta_max: float = 20.0, num_scales: int = 1000):
...
class VPSDE(nn.Module):
  ...
  def prior_logp(self, z: Tensor) -> Tensor:
    shape = z.shape
    N = np.prod(shape[1:])
    logps = -N / 2.0 * np.log(2 * np.pi) - (z ** 2).view(z.size(0), -1).sum(dim=-1) / 2.0
    return logps
...
class VPAncestralSamplingPredictor(object):
  """The ancestral sampling predictor. Only supports VP SDE."""
  def __init__(self, sde: VPSDE):
    self.sde = sde
 
  def update_fn(self, x: Tensor, score: Tensor, t: Tensor, noise: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
