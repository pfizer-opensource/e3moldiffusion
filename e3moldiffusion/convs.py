...
class EQGATEdgeConv(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        # symmetric function
        self.edge_net = nn.Sequential(DenseLayer(1 * self.si + edge_dim + 1,
                                                 self.si,
                                                 bias=True, activation=nn.SiLU()
                                                 ),
                                      DenseLayer(self.si, edge_dim + self.v_mul * self.vi + self.si,
                                                 bias=True
                                                 )
                                      )
        self.scalar_net = DenseLayer(self.si, self.si, bias=True)
    ...
    def message(
        ...
        aij = self.edge_net(aij)
        fdim = aij.shape[-1]
        aij, e = aij.split([fdim - self.edge_dim, self.edge_dim], dim=-1)
...
class EQGATEdgeRBFConv(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        super(EQGATEdgeRBFConv, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
        self.vector_aggr = vector_aggr
        self.in_dims = in_dims
        ...
        # symmetric function
        self.edge_net = nn.Sequential(DenseLayer(1 * self.si + self.edge_dim + rbf_dim + 1,
                                                 self.si,
                                                 bias=True, activation=nn.SiLU()
                                                 ),
                                      DenseLayer(self.si, self.edge_dim + self.v_mul * self.vi + self.si,
                                                 bias=True
                                                 )
                                      )
        self.scalar_net = DenseLayer(self.si, self.si, bias=True)
...
# Edges but modelling it SE(3) equivariant, i.e. also including translation equivariance

class EQGATEdgeConvV2(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        super(EQGATEdgeConvV2, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
        self.vector_aggr = vector_aggr
        self.in_dims = in_dims
        ...
        self.edge_net = nn.Sequential(DenseLayer(1 * self.si + edge_dim + 1,
                                      ...
                                      DenseLayer(self.si, edge_dim + self.v_mul * self.vi + self.si + 1,
                                                 bias=True
                                                 )
                                      )
        self.scalar_net = DenseLayer(self.si, self.si, bias=True)
    ...
    def message(
        ...
        aij = self.edge_net(aij)
        fdim = aij.shape[-1]
        aij, e, gij = aij.split([fdim - self.edge_dim - 1, self.edge_dim, 1], dim=-1)
        ...
        return ns_j, nv_j, pj, e
    
class EQGATEdgeRBFConvV2(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        self.edge_net = nn.Sequential(DenseLayer(1 * self.si + self.edge_dim + rbf_dim + 1,
                                      ...
                                      DenseLayer(self.si, self.edge_dim + self.v_mul * self.vi + self.si + 1,
                                                 bias=True
                                                 )
                                      )
        self.scalar_net = DenseLayer(self.si, self.si, bias=True)
...
class EQGATRBFConvV2(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        super(EQGATRBFConvV2, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
        self.vector_aggr = vector_aggr
        self.in_dims = in_dims
        ...
        self.edge_net = nn.Sequential(DenseLayer(2 * self.si + self.edge_dim + rbf_dim + 1,
                                      ...
                                      DenseLayer(self.si, self.v_mul * self.vi + self.si + 1,
                                                 bias=True
                                                 )
                                      )
        self.scalar_net = DenseLayer(self.si, self.si, bias=True)
...
class EQGATConvV2(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        super(EQGATConvV2, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
        self.vector_aggr = vector_aggr
        self.in_dims = in_dims
...
class EQGATGlobalConv(MessagePassing):
    def __init__(
        self,
        in_dims: Tuple[int, Optional[int]],
        out_dims: Tuple[int, Optional[int]],
        ...
        super(EQGATGlobalConv, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
...
class EQGATGlobalConvNoEdge(MessagePassing):
    def __init__(
        self,
        in_dims: Tuple[int, Optional[int]],
        out_dims: Tuple[int, Optional[int]],
        ...
        super(EQGATGlobalConvNoEdge, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
...
class EQGATLocalConv(MessagePassing):
    """
    Slightly modified SO(3) equivariant graph attention convolution described in
    @inproceedings{
        le2022representation,
    ...
    def __init__(
        ...
        super(EQGATLocalConv, self).__init__(
            node_dim=0, aggr=None, flow="source_to_target"
        )
        self.vector_aggr = vector_aggr
        self.in_dims = in_dims
