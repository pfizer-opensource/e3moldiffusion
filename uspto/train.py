...
from uspto.utils import _collate
from uspto.utils_data import save_pickle

from e3moldiffusion.molfeat import get_bond_feature_dims
from e3moldiffusion.coordsatoms_uspto import ScoreAtomsModel
...
class Trainer(pl.LightningModule):
    ...
    def step(self, batch: Batch, stage: str):
        ...
        reactants_embed = batch.reactants_embed
        ...
        if self.hparams["classifier_free_reactants_guidance"]:
            reactants_embed[
                np.where(np.random.rand(bs) < self.hparams["guidance_prob_reactants"])
            ] = 0
    ...
    def sample_p_xh_given_z0(
        ...
        if self.hparams["classifier_free_guidance"]:
            ...
            eps_t0_cnull = self.network(
                z0, zeros, batch, reactants_embed, context=context
            )
    ...
    def compute_loss(
        ...
        eps_t = self.network(z_t, t, batch, reactants_embed, context=context)
        # Compute the error.
    ...
    def sample_p_zs_given_zt(
        ...
        # Neural net prediction.
        eps_t = self.network(zt, t, batch, reactants_embed, context=context)
        if self.hparams["classifier_free_guidance"]:
            assert context is not None
            context = torch.zeros_like(context, device=context.device)
            eps_t_cnull = self.network(zt, t, batch, reactants_embed, context=context)
    ...
    @torch.no_grad()
    def generate_chain(self, n_nodes, reactants_embed, context=None, keep_frames=100):
        """
    ...
    def analyze_and_save(
        ...
        if path is None:
            csv_path = os.path.join(self.hparams["log_dir"], "rdkit_eval.csv")
            mol_path = os.path.join(self.hparams["log_dir"], "molecules.pickle")
        else:
            csv_path = os.path.join(path, "rdkit_eval.csv")
            mol_path = os.path.join(path, "molecules.pickle")
        ...
        for _ in range(int(n_samples / batch_size)):
            ...
            # nodesxsample = torch.tensor([20] * batch_size)
        ...
        write_to_csv(
            ...
            midi_log,
            csv_path,
            include_charges=self.hparams["include_charges"],
        )
        save_pickle(molecule_list, mol_path)
