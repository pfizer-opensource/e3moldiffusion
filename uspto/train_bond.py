...
from uspto.analyze_sampling import SamplingMetrics
from uspto.info_data import USPTOInfos
from uspto.utils_metrics import batch_to_list
from uspto.utils_sampling import (
    check_stability_given_bonds,
    Molecule,
)
from uspto.utils import PlaceHolder
from uspto.visualize_bonds import visualize_mol, visualize_chain, visualize
...
class Trainer(pl.LightningModule):
    ...
    def step(self, batch: Batch, stage: str):
        ...
        reactants_embed: Tensor = batch.reactants

        edge_index_global, dense_edge_oh = self.get_fully_connected_edge_attr(batch)
    ...
    def network(
        ...
        edge_attr_global = (
            ...
            if self.hparams["fully_connected_layer"]
            else None
        )
        edge_attr_local = (
    ...
    def compute_error(self, net_out, edge_out, eps, eps_edge, batch, source):
        ...
        bonds_loss = scatter_mean(
            scatter_mean(bonds_loss, index=source, dim=0), index=batch, dim=0
        )
        loss_dict = {
    ...
    def sample_p_xh_given_z0(
        ...
        size = torch.Size([num_nodes, num_nodes] + list(eps_edge_t0.size())[1:])
        adj = torch.sparse_coo_tensor(edge_index_global, eps_edge_t0, size)
        eps_edge_t0 = adj.to_dense()
        ...
        dense_edge_oh = self.sample_symmetric_edge_normal(mu_bond, sigma_x)
    ...
    def sample_symmetric_edge_normal(self, mu_edge, sigma, fix_noise=False):
        dense_noise = self.sample_symmetric_edge_noise(mu_edge)
    ...
    def log_pxh_given_z0_without_constants(
        ...
        loss_dict = self.compute_error(net_x, edge_out, eps_x, eps_edge, batch, source)
        ...
        log_pedge_cat = torch.sum(
            scatter_add(
                scatter_add(log_probabilities_e * edge_out, source, dim=0), batch, dim=0
            ),
            dim=1,
        )
    ...
    def compute_loss(
        ...
        loss_dict = self.compute_error(
            eps_t, eps_edge_t, noise_xh, noise_edge, data_batch, source
        )
        error = (
            loss_dict["coords_loss"]
            + loss_dict["atoms_loss"]
    ...
    def sample_p_zs_given_zt(
        ...
        size = torch.Size([num_nodes, num_nodes] + list(eps_edge_t.size())[1:])
        adj = torch.sparse_coo_tensor(edge_index_global, eps_edge_t, size)
        ...
        mu_edge = (
            ...
            - (sigma2_t_given_s / alpha_t_given_s.unsqueeze(1) / sigma_t.unsqueeze(1))
            * eps_edge_t
        )
        ...
        edge_s = self.sample_symmetric_edge_normal(mu_edge, sigma, fix_noise)
    ...
    def sample_chain(self, n_tries, reactants_embed=None, context=None):
        n_samples = 1
        if (
        ...
        for i in range(n_tries):
            chain, chain_bonds = self.generate_chain(
                n_nodes,
                reactants_embed=reactants_embed,
                context=context,
                keep_frames=100,
...
if __name__ == "__main__":
    from uspto.data import USPTODataModule
    from uspto.hparams import add_arguments
    ...
    datamodule = USPTODataModule(hparams)
    datamodule.prepare_data()
    datamodule.setup("fit")

    dataset_statistics = USPTOInfos(datamodule.dataset)
