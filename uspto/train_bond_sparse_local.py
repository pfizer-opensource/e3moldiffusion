...
class Trainer(pl.LightningModule):
    ...
    def network(
        ...
        with torch.set_grad_enabled(self.training or self.hparams["energy_preserving"]):
            ...
            return net_out, edge_pred, edge_pred_l
    ...
    def compute_error(
        ...
        if self.hparams["use_local_edge_attr"]:
            bonds_loss_l = torch.pow(edge_out_local - noise_edge_true_local, 2).mean(-1)
            bonds_loss_l = scatter_mean(bonds_loss_l, index=batch[source_local], dim=0)
            bonds_loss = bonds_loss + bonds_loss_l
    ...
    def sample_p_xh_given_z0(
        ...
        eps_t0, eps_edge_t0, _ = self.network(
            x=z_0,
            t=zeros,
            data_batch=data_batch,
            edge_index_local=edge_index_local,
            edge_index_global=edge_index_global,
            edge_attr_local=edge_attr_local_t0,
    ...
    def log_pxh_given_z0_without_constants(
        ...
        loss_dict = self.compute_error(
            ...
            batch,
            source,
            noise_edge_true_local=eps_edge_local,
            edge_out_local=edge_out_local,
    ...
    def compute_loss(
        ...
        if self.hparams["use_local_edge_attr"]:
            edge_attr_local = self.dense_to_sparse(dense_edge_oh, edge_index_local)
            noise_edge_oh_local = self.dense_to_sparse(noise_edge_oh, edge_index_local)
            edge_attr_local_t = (
                alpha_t[edge_index_local[0]] * edge_attr_local
                + sigma_t[edge_index_local[0]] * noise_edge_oh_local
            )
        else:
            edge_attr_local_t = None

        # Neural net prediction.
        eps_t, eps_edge_t, eps_edge_t_local = self.network(
            z_t,
            t,
            data_batch,
            edge_index_local,
            edge_index_global,
            edge_attr_local=edge_attr_local_t,
        ...
        loss_dict = self.compute_error(
            ...
            data_batch,
            source,
            edge_out_local=eps_edge_t_local,
            noise_edge_true_local=noise_edge_oh_local,
            source_local=edge_index_local[0],
        )
        error = (
            loss_dict["coords_loss"]
            + loss_dict["atoms_loss"]
        ...
        if t0_always:
            ...
            if self.hparams["use_local_edge_attr"]:
                noise_edge_oh_local_0 = self.dense_to_sparse(
                    noise_edge_oh_0, edge_index_local
                )
                edge_attr_local_t0 = (
                    alpha_0[edge_index_local[0]] * edge_attr_local
                    + sigma_0[edge_index_local[0]] * noise_edge_oh_local_0
                )
            else:
                edge_attr_local_t0 = None

            eps_t0, eps_edge_t0, eps_edge_t0_local = self.network(
                z_0,
                t_zeros,
                data_batch,
                edge_index_local,
            ...
            loss_term_0 = -self.log_pxh_given_z0_without_constants(
                ...
                batch=data_batch,
                source=source,
                eps_edge_local=noise_edge_oh_local_0,
                edge_out_local=eps_edge_t0_local,
                source_local=edge_index_local[0],
            )
    ...
    def sample_p_zs_given_zt(
        ...
        # Neural net prediction.
        eps_t, eps_edge_t, eps_edge_t_l = self.network(
            x=z_t,
            t=t,
            data_batch=data_batch,
            edge_index_local=edge_index_local,
        ...
        mu_edge_l = (
            (
                edge_attr_local_t / alpha_t_given_s[source_l]
                - (
                    sigma2_t_given_s[source_l]
                    / alpha_t_given_s[source_l]
                    / sigma_t[source_l]
                )
                * eps_edge_t_l
            )
        ...
        edge_s_l = (
            self.sample_symmetric_edge_normal(
                mu_edge_l, sigma[source_l], edge_index_local, sample_size, fix_noise
            )
            if self.hparams["use_local_edge_attr"]
            else None
        )
    ...
    def generate_sample(
        ...
        edge_attr_local_t = (
            self.dense_to_sparse(noise_edge_oh, edge_index_local)
            if self.hparams["use_local_edge_attr"]
            else None
        )
        ...
        for s in reversed(range(0, self.T)):
            ...
            (
                ...
                edge_index_local=edge_index_local,
                edge_index_global=edge_index_global,
                edge_attr_local_t=edge_attr_local_t,
        ...
        x, h = self.sample_p_xh_given_z0(
            ...
            edge_index_local=edge_index_local,
            edge_index_global=edge_index_global,
            edge_attr_local_t0=edge_attr_local_t,
    ...
    def generate_chain(
        ...
        chain_bonds = torch.zeros(
            (keep_frames,) + edge_attr_global_t.size(), device=z.device
        )
        ...
        for s in reversed(range(0, self.T)):
            ...
            z, edge_attr_global_t, edge_attr_local_t = self.sample_p_zs_given_zt(
                s=s_array,
                t=t_array,
                z_t=z,
                data_batch=batch,
            ...
            chain[write_index], chain_bonds[write_index] = (
                self.unnormalize_z(z, edge_attr_global_t)[0],
                self.unnormalize_z(z, edge_attr_global_t)[1],
            )
