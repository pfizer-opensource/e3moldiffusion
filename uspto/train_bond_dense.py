...
from aqm.utils_metrics import batch_to_list
from aqm.utils_sampling import (
    sanitize_molecules_openbabel,
    check_stability_given_bonds,
    Molecule,
)
from aqm.utils import PlaceHolder
from aqm.visualize_bonds import visualize_mol, visualize_chain, visualize
...
class Trainer(pl.LightningModule):
    ...
    def get_fully_connected_edge_attr(self, batch):
        ...
        fc_edge_index, fc_edge_attr = sort_edge_index(
            fc_edge_index, fc_edge_attr, sort_by_row=False
        )

        dense_edge = torch.zeros(n, n, dtype=torch.long, device=fc_edge_index.device)
        node_mask = torch.zeros(n, n, dtype=torch.bool, device=fc_edge_index.device)
        node_mask[fc_edge_index[0, :], fc_edge_index[1, :]] = 1

        dense_edge[fc_edge_index[0, :], fc_edge_index[1, :]] = fc_edge_attr
        dense_edge_oh = (
            F.one_hot(dense_edge.view(-1, 1), num_classes=5).view(n, n, -1).float()
        )
        return fc_edge_index, dense_edge_oh, node_mask

    def create_node_mask(self, data_batch):
        n_nodes = data_batch.bincount()
        batch_size = len(n_nodes)
        n_max = torch.sum(n_nodes).item()

        arange = torch.arange(n_max, device=self.device).unsqueeze(0)
        node_mask = arange < n_nodes.unsqueeze(1)
        diag_mask = ~torch.eye(
            node_mask.size(0), device=node_mask.device, dtype=torch.bool
        ).unsqueeze(0)
        node_mask = node_mask.unsqueeze(1) * node_mask.unsqueeze(2) * diag_mask
        return node_mask
    ...
    def network(
        ...
        edge_attr_global = (
            self.dense_to_sparse(dense_edge, edge_index_global)
            if self.hparams["fully_connected_layer"] or self.hparams["fully_connected"]
            else None
        )
        edge_attr_local = (
            self.dense_to_sparse(dense_edge, edge_index_local)
        ...
        with torch.set_grad_enabled(self.training or self.hparams["energy_preserving"]):
            ...
            num_nodes = data_batch.bincount().sum()
            size = torch.Size([num_nodes, num_nodes] + list(edge_pred.size())[1:])
            adj = torch.sparse_coo_tensor(edge_index_global, edge_pred, size)
            eps_edge_t = adj.to_dense()

            mask = torch.zeros((num_nodes, num_nodes, 1), device=eps_edge_t.device)
            mask[edge_index_local[0], edge_index_local[1]] = 1

            eps_edge_t = eps_edge_t * mask

            return net_out, eps_edge_t, mask
    ...
    def compute_x_pred(self, net_out, edge_out, z_t, edge_t, gamma_t, data_batch):
        ...
        if self.parametrization == "x":
            ...
            bond_pred = (
                1.0 / alpha_t.unsqueeze(1) * (edge_t - sigma_t.unsqueeze(1) * edge_out)
            )
        else:
    ...
    def compute_error(self, net_out, edge_out, eps, eps_edge, batch):
        """Computes error, i.e. the most likely prediction of x."""
        # eps_t = net_out

        # if self.training and self.loss_type == "l2":
        #     denom = (self.n_dims + self.num_atom_features) * torch.bincount(batch)
        #     error_coord_atom = torch.sum(
        #         scatter_add((eps - eps_t) ** 2, batch, dim=0) / denom.unsqueeze(1),
        #         dim=1,
        #     )
        #     error_edge = torch.sum(
        #         scatter_add(
        #             scatter_add((eps_edge - edge_out) ** 2, source, dim=0)
        #             / denom[batch].unsqueeze(1),
        #             batch,
        #             dim=0,
        #         ),
        #         dim=1,
        #     )
        #     # error_edge = torch.pow(edge_out - eps_edge, 2).mean(-1)
        #     # error_edge = error_edge.mean()
        # else:
        #     error_coord_atom = torch.sum(
        #         scatter_add((eps - eps_t) ** 2, batch, dim=0), dim=1
        #     )
        #     error_edge = scatter_add(
        #         torch.sum(
        #             scatter_add((eps_edge - edge_out) ** 2, source, dim=0), dim=1
        #         ),
        #         batch,
        #         dim=0,
        #     )
        ...
        coords_pred, coords_true = net_out[:, : self.n_dims], eps[:, : self.n_dims]
        atoms_pred, atoms_true = (
            net_out[:, self.n_dims : -1]
            if self.include_charges
            else net_out[:, self.n_dims :],
            eps[:, self.n_dims : -1] if self.include_charges else eps[:, self.n_dims :],
        )
        charges_pred, charges_true = (
            net_out[:, -1:] if self.include_charges else None,
            eps[:, -1:] if self.include_charges else None,
        )
        bonds_pred, bonds_true = edge_out, eps_edge
        ...
        # num_edges = bonds_true[:, :, 0].nonzero().shape[0]
        num_nodes = bonds_pred.shape[0]
        bonds_loss = torch.pow(bonds_pred - bonds_true, 2).mean(-1)
        bonds_loss = torch.sum(bonds_loss, dim=1) / num_nodes
    ...
    def sample_p_xh_given_z0(
        ...
        eps_t0, eps_edge_t0, edge_mask = self.network(
            x=z_0,
            t=zeros,
            data_batch=data_batch,
            dense_edge=dense_edge_0,
            edge_index_global=edge_index_global,
            context=context,
        )
        ...
        dense_edge_oh = self.sample_symmetric_edge_normal(mu_bond, sigma_x, edge_mask)
        ...
        x, h_cat, h_int, dense_edge_oh = self.unnormalize(
            x, h_cat, h_int, dense_edge_oh
        )

        h_cat = F.one_hot(torch.argmax(h_cat, dim=1), num_classes=self.num_classes)
        h_edge = F.one_hot(torch.argmax(dense_edge_oh, dim=2), num_classes=5)
    ...
    def sample_symmetric_edge_normal(self, mu_edge, sigma, edge_mask, fix_noise=False):
        dense_noise = self.sample_symmetric_edge_noise(mu_edge) * edge_mask
        return mu_edge + sigma * dense_noise
    ...
    def log_pxh_given_z0_without_constants(
        ...
        batch,
        gamma,
        node_mask,  # CHANGE to node_mask, if global model (edge_mask is local)
        epsilon=1e-10,
    ):
        if node_mask.dim() < 3:
        ...
        loss_dict = self.compute_error(net_x, edge_out, eps_x, eps_edge, batch)
        ...
        edge_onehot = dense_edge_oh * self.norm_values[3] + self.norm_biases[3]
        ...
        centered_edge_cat = centered_edge_cat * node_mask
        ...
        log_pedge_cat_proportional = (
            torch.log(
                cdf_standard_gaussian(
                    (centered_edge_cat + 0.5) / sigma_0_cat.unsqueeze(1)
                )
                * node_mask
                - cdf_standard_gaussian(
                    (centered_edge_cat - 0.5) / sigma_0_cat.unsqueeze(1)
        ...
        log_Z_e = (
            torch.logsumexp(log_pedge_cat_proportional, dim=2, keepdim=True) * node_mask
        )
        ...
        num_nodes = torch.sum(batch.bincount())
        log_pedge_cat = torch.sum(
            scatter_add(
                torch.sum(log_probabilities_e * edge_onehot * node_mask, dim=1)
                / num_nodes,
                batch,
                dim=0,
            ),
    ...
    def compute_loss(
        ...
        # Edge noise
        noise_edge_oh = self.sample_symmetric_edge_noise(
            dense_edge_oh=dense_edge_oh, node_mask=node_mask
        )
        z_t = alpha_t * xh + sigma_t * noise_xh
        dense_edge_oh_t = (
            alpha_t.unsqueeze(1) * dense_edge_oh + sigma_t.unsqueeze(1) * noise_edge_oh
        )
        dense_edge_oh_t = dense_edge_oh_t * node_mask.unsqueeze(-1)
        # transform edge noise to PyG format
        # noise_edge = self.dense_to_sparse(noise_edge_oh, edge_index_global)

        assert_mean_zero(z_t[:, : self.n_dims], data_batch)

        # Neural net prediction.
        eps_t, eps_edge_t, edge_mask = self.network(
            z_t,
            t,
            data_batch,
            edge_index_global,
        ...
        loss_dict = self.compute_error(
            eps_t, eps_edge_t, noise_xh, noise_edge_oh, data_batch
        )
        error = (
            loss_dict["coords_loss"]
            + loss_dict["atoms_loss"]
        ...
        if t0_always:
            ...
            dense_edge_oh_t0 = (
                alpha_t.unsqueeze(1) * dense_edge_oh
                + sigma_t.unsqueeze(1) * noise_edge_oh_0
            )  ### ERROR - EITHER ON alpha_t[source] or diag_embed(ahlpas)
            dense_edge_oh_t0 = dense_edge_oh_t0 * node_mask.unsqueeze(-1)
            # Transform edge noise PyG format
            # noise_edge_0 = self.dense_to_sparse(noise_edge_oh_0, edge_index_global)
            ...
            loss_term_0 = -self.log_pxh_given_z0_without_constants(
                ...
                eps=noise_xh_0,
                net_out=eps_t0,
                eps_edge=noise_edge_oh_0,
                edge_out=eps_edge_t0,
                gamma=gamma_0,
                node_mask=edge_mask,  # CHANGE to node_mask, if global model (edge_mask is local)
                batch=data_batch,
            )
    ...
    def sample_p_zs_given_zt(
        ...
        mu_edge = (
            dense_edge_t / alpha_t_given_s.unsqueeze(1)
            - (
        ...
        edge_s = self.sample_symmetric_edge_normal(
            mu_edge, sigma.unsqueeze(1), edge_mask, fix_noise
        )
    ...
    def sample_symmetric_edge_noise(self, dense_edge_oh, node_mask):
        noise_edges = torch.randn_like(dense_edge_oh)
        noise_edges = 0.5 * (noise_edges + noise_edges.permute(1, 0, 2))
        noise_edges = noise_edges * node_mask.unsqueeze(-1)
        return noise_edges
    ...
    def generate_sample(
        ...
        dense_edge = self.sample_symmetric_edge_noise(dense_edge_oh=dense_edge_oh)
        ...
        for s in reversed(range(0, self.T)):
            ...
            z, dense_edge = self.sample_p_zs_given_zt(
                s=s_array,
                t=t_array,
                z_t=z,
                dense_edge_t=dense_edge,
                data_batch=batch,
                edge_index_global=edge_index_global,
                num_nodes=num_nodes,
                context=context,
    ...
    def generate_chain(
        ...
        chain_bonds = torch.zeros((keep_frames,) + dense_edge.size(), device=z.device)
        ...
        for s in reversed(range(0, self.T)):
            ...
            chain[write_index], chain_bonds[write_index] = (
                self.unnormalize_z(z, dense_edge)[0],
                self.unnormalize_z(z, dense_edge)[1],
            )
    ...
    def sample_chain(self, n_tries, context=None):
        ...
        for i in range(n_tries):
            ...
            chain = PlaceHolder(
                ...
                charges=charges,
                bond_types=chain_bonds.argmax(3),
                pos=x,
            )
            if mol_stable:
