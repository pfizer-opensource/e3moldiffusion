...
from e3moldiffusion.gsm import NonConservativeScoreNetwork, exact_jacobian_trace, SimplePotentialScoreNetwork
...
class Trainer(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if self.simple:
            self.model = SimplePotentialScoreNetwork(hn_dim=(hparams["sdim"], hparams["vdim"]),
    ...
    def step_fnc(self, batch, batch_idx, stage):         
        ...
        jcb_tr = scatter_add(src=jcb_tr, index=batch.batch, dim=0, dim_size=batch_size) # [B,]
        #print(f"before reduce: {jcb_tr.shape}: ", jcb_tr)
        jcb_tr = jcb_tr.mean(0)
        #print("after reduce: ", jcb_tr)
        #time.sleep(1)
        #print()
        # jcb_tr = 0.0
        snorm = torch.pow(score, 2).sum(-1)
        snorm = scatter_add(src=snorm, index=batch.batch, dim=0, dim_size=batch_size)
        snorm = snorm.mean(0)
        # snorm = -1.0 * snorm.mean(0)
        #snorm = 0.0
        # print(snorm)
        # try just optimizing for divergence. should go to neg. infty
        #snorm = 0.0
        # jcb_tr = jcb_tr.mean()
        # jcb_tr = 0.0
