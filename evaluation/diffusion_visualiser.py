...
def save_xyz_file(
    ...
    if batch is None:
        ...
        num_atoms_prev = 0
        for i, num_atoms in enumerate(atomsxmol):
            f = open(path + name + "_" + "%03d.txt" % (i + id_from), "w")
            f.write("%d\n\n" % atomsxmol[i])
            atoms = torch.argmax(
                one_hot[num_atoms_prev : num_atoms_prev + num_atoms], dim=1
            )
...
if __name__ == "__main__":
    ...
    from config_file import qm9_with_h, geom_with_h
