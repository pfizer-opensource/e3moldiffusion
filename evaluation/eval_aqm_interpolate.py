...
def test_conditional(
    ...
    for num_atoms in [60, 70, 80, 90]:
        start = time.time()
        print(f"Start interpolation...")
        model.save_and_sample_conditional(
            path=f"{output_path}/interpolate_{interpolate_prop}_natoms_{num_atoms}/",
            n_nodes=num_atoms,
            n_frames=100,
            interpolate_prop=interpolate_prop,
            interpolate_range=None,
            use_validity_filtering=True,
            n_samples=5000,
            max_iter=1000,
            id_from=0,
        )
        ...
        vis.visualize_chain(
            f"{output_path}/interpolate_{interpolate_prop}_natoms_{num_atoms}/",
            model.dataset_info,
            wandb=None,
            mode="conditional",
        )
    ...
    # start = time.time()
    # print(f"Start interpolation in a given range...")
    # model.save_and_sample_conditional(
    #     path=f"{output_path}/interpolate_{interpolate_prop}_range{interpolate_range}_natoms_{num_atoms}/",
    #     n_nodes=num_atoms,
    #     n_frames=100,
    #     interpolate_prop=interpolate_prop,
    ...
    # print(f"Interpolation took {time.time() - start:.2f} seconds")
    # vis.visualize_chain(
    #     f"{output_path}/interpolate_{interpolate_prop}_range{interpolate_range}_natoms_{num_atoms}/",
    #     model.dataset_info,
    #     wandb=None,
    #     mode="conditional",
    # )
    ...
    # start = time.time()
    # print(f"Start interpolation without validity check...")
    # model.save_and_sample_conditional(
