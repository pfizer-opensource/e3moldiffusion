...
from evaluation.ligand_builder import get_bond_order
...
def save_xyz_file_ligand_pocket(
        path, one_hot, one_hot_pocket, positions, positions_pocket, dataset_info, id_from=0,
                  name='molecule', batch_mask=None, batch_mask_pocket=None
                  ):
    try:
        os.makedirs(path)
    except OSError:
    ...
    if batch_mask is None:
        batch_mask = torch.zeros(len(one_hot))
    if batch_mask_pocket is None:
        batch_mask_pocket = torch.zeros(len(one_hot_pocket))

    for batch_i in torch.unique(batch_mask):
        cur_batch_mask = (batch_mask == batch_i)
        cur_batch_mask_pocket = (batch_mask_pocket == batch_i)
        n_atoms_lig = int(torch.sum(cur_batch_mask).item())
        n_atoms_pocket = int(torch.sum(cur_batch_mask_pocket).item())
        n_atoms = n_atoms_lig + n_atoms_pocket
        f = open(path + name + '_' + "%03d.xyz" % (batch_i + id_from), "w")
        f.write("%d\n\n" % n_atoms)
        atoms_lig = torch.argmax(one_hot[cur_batch_mask], dim=1)
        atoms_pocket = torch.argmax(one_hot_pocket[cur_batch_mask_pocket], dim=1)
        atoms = torch.cat([atoms_lig, atoms_pocket])
        batch_pos_lig = positions[cur_batch_mask]
        batch_pos_pocket = positions_pocket[cur_batch_mask_pocket]
        batch_pos = torch.cat([batch_pos_lig, batch_pos_pocket])
        for atom_i in range(n_atoms):
            atom = atoms[atom_i]
            atom = dataset_info['atom_decoder'][atom] if atom_i < n_atoms_lig else dataset_info['atom_decoder_pocket'][atom]
            f.write("%s %.9f %.9f %.9f\n" % (atom, batch_pos[atom_i, 0], batch_pos[atom_i, 1], batch_pos[atom_i, 2]))
        f.close()
...
def load_molecule_xyz(file, dataset_info):
    with open(file, encoding='utf8') as f:
        ...
        one_hot_pocket = torch.zeros(n_atoms, len(dataset_info['atom_decoder_pocket']))
        ...
        for i in range(n_atoms):
            ...
            try:
                ...
                one_hot_pocket[i, dataset_info['atom_encoder_pocket'][atom_type]] = 1
            position = torch.Tensor([float(e) for e in atom[1:]])
            positions[i, :] = position
        return positions, one_hot, one_hot_pocket
...
def visualize(path, dataset_info, max_num=25, wandb=None, spheres_3d=False):
    ...
    for file in files:
        positions, one_hot, one_hot_pocket = load_molecule_xyz(file, dataset_info)
        atom_type_ligand = torch.argmax(one_hot, dim=1)
        atom_type_pocket = torch.argmax(one_hot_pocket, dim=1)
        atom_type = torch.cat([atom_type_ligand, atom_type_pocket]).numpy()
