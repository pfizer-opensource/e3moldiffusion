...
from aqm.utils_data import Statistics
...
def edge_counts(data_list, num_bond_types=5):
    ...
    for data in data_list:
        ...
        num_edges = data.bond_attr.shape[0]
        num_non_edges = total_pairs - num_edges
        assert num_non_edges >= 0

        edge_types = (
            torch.nn.functional.one_hot(
                data.bond_attr - 1, num_classes=num_bond_types - 1
            )
            .sum(dim=0)
            .numpy()
        )
...
def bond_lengths_counts(data_list, num_bond_types=5):
    ...
    for data in data_list:
        ...
        bond_distances = cdists[data.bond_index[0], data.bond_index[1]]
        for bond_type in range(1, num_bond_types):
            bond_type_mask = data.bond_attr == bond_type
...
def bond_angles(data_list, atom_encoder):
    ...
    for data in data_list:
        ...
        for i in range(data.num_nodes):
            neighbors = data.bond_index[1][data.bond_index[0] == i]
            for j in neighbors:
                for k in neighbors:
                    if j == k:
                        continue
