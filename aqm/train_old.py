...
from e3moldiffusion.gnn_cond import EQGATEncoder
...
class Trainer(pl.LightningModule):
    def __init__(
        ...
        self.in_node_nf = self.hparams.num_atom_types + int(self.include_charges)
        ...
        if self.hparams.model == "equivariant-transformer":
            ...
            self.model = EQGATEncoder(
                ...
                num_classes=hparams["num_atom_types"],
                num_rbf=hparams["num_rbf"],
                use_rbf=hparams["use_rbf"],
                cutoff=hparams["cutoff_upper"],
    ...
    def step(self, batch: Batch, stage: str):
        ...
        if self.properties_norm is not None:
            ...
            context_node_nf = context.size(1)
        else:
            context = None
            context_node_nf = 0
        ...
        Q = (torch.zeros(0) if not self.hparams["include_charges"] else batch.Q).to(
            node_feat.device
        )
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if (self.current_epoch + 1) % self.hparams["test_interval"] == 0:
            ...
            self.analyze_and_save(
                n_samples=self.hparams["n_stability_samples"],
            )
    ...
    def network(
        ...
        z = x[:, self.n_dims : -1] if self.include_charges else x[:, self.n_dims :]
        ...
        if self.hparams["fully_connected"] or self.hparams["fully_connected_layer"]:
            edge_index_global = torch.eq(
                batch.unsqueeze(0), batch.unsqueeze(-1)
            ).int()  # .fill_diagonal_(0)
        ...
        with torch.set_grad_enabled(self.training or self.hparams["energy_preserving"]):
            ...
            noise_pred = remove_mean(net_out["vector"], batch)
            if self.training:
            ...
            net_out = torch.cat([noise_pred, net_out["scalar"]], dim=1)
            return net_out
    ...
    def compute_error(self, net_out, gamma_t, eps, batch):
        ...
        if self.training and self.loss_type == "l2":
            denom = (self.n_dims + self.in_node_nf) * torch.bincount(batch)
            error = torch.sum(
    ...
    def compute_loss(
        ...
        if self.hparams["continuous"]:
            t = torch.rand(
                size=(len(batch.unique()),),
                dtype=x.dtype,
                device=x.device,
            )
            t_cont = (
                t * (self.sde.T - self.hparams["eps_min"]) + self.hparams["eps_min"]
            )
        else:
    ...
    def sample_sweep_conditional(
        ...
        if use_openbabel:
            ...
            one_hot, charges, x, batch, _, failed_ids = sanitize_molecules_openbabel(
                one_hot, charges, x, batch, self.dataset_info
            )
            context = torch.cat(
                [c for i, c in enumerate(context_list) if i not in failed_ids],
                dim=0,
            )
        else:
            one_hot, charges, x, batch = self.sample(
    ...
    def sample_and_interpolate_conditional(
        self,
        path,
        n_nodes=50,
        n_frames=10,
        ...
        if interpolate_range is None:
            ...
            min_val, max_val = self.prop_dist.distributions[interpolate_prop][n_nodes][
                "params"
            ]
            mean, mad = (
        ...
        iter = 0
        valid_samples = 0
        while valid_samples < n_samples and iter < max_iter:
            iter += 1
            ...
            if use_openbabel:
                ...
                ), "If use_openbabel is set to true, max_iter iterations are start per sample of n_samples!"
                ...
                index = torch.ones(context.shape[0], dtype=bool)
                index[failed_ids] = False
                selected_context = context[index]

                valid_samples += n_molecules
            else:
                valid_samples += len(one_hot)

            final_path = os.path.join(path, str(iter - 1))
            vis.save_xyz_file(
                final_path,
                one_hot,
                charges,
            ...
            with open(os.path.join(final_path, f"context.pickle"), "wb") as f:
                pickle.dump(selected_context.cpu(), f)
    ...
    def analyze_and_save(
        ...
        for _ in range(int(n_samples / batch_size)):
            ...
            molecules["batch"].append(batch.detach().cpu())

        if use_openbabel:
            molecules = sanitize_molecules_openbabel_batch(molecules, self.dataset_info)

        validity_dict, rdkit_tuple = analyze_stability_for_molecules(
            molecules, self.dataset_info
        )
        if rdkit_tuple is not None:
            print(
                f"\n'Molecule stability': {validity_dict['mol_stable']}, 'Atom stability': {validity_dict['atm_stable']}"
            )
            print(
                f"\n'Validity': {rdkit_tuple[0][0]}, 'Uniqueness': {rdkit_tuple[0][1]}, 'Novelty': {rdkit_tuple[0][2]}\n"
            )
        write_to_csv(validity_dict, rdkit_tuple, path)
