...
from aqm.utils_sampling import Molecule
...
def visualize_mol(
    ...
    all_file_paths = []
    if log == "graph":
        pos = pca.fit_transform(molecule.positions)
        molecule.positions = torch.from_numpy(pos).to(molecule.atom_types.device)
    file_path = os.path.join(path, f"{file_prefix}.png")
    plot_save_molecule(molecule, save_path=file_path, conformer2d=conformer2d)
    all_file_paths.append(file_path)
...
def generatePIL2d(mol, conformer2d=None):
    ...
    if conformer2d is not None:
        ...
        for j, atom in enumerate(mol.GetAtoms()):
            x, y, z = (
                conformer2d[j, 0].item(),
                conformer2d[j, 1].item(),
                conformer2d[j, 2].item(),
            )
...
def visualize_chain(path, chain, atom_decoder):
    ...
    print(f"Visualizing chain...\n")
    result_path = os.path.join(path, f"chain")

    chain_atoms = chain.atom_types.argmax(-1).long()
    chain_charges = chain.charges
    chain_bonds = chain.bond_types.argmax(-1).long()
    chain_positions = chain.pos
    ...
    for j in range(chain_atoms.shape[0]):
        ...
        mols.append(
            Molecule(
                atom_types=chain_atoms[j],
                charges=chain_charges[j],
                bond_types=chain_bonds[j],
    ...
    for frame in range(len(mols)):
        all_file_paths = visualize(
            result_path,
            mols,
            num_molecules_to_visualize=-1,
            log=None,
            conformer2d=conformer2d,
            file_prefix="frame",
        )
...
def plot_molecule3d(
    ax, positions, atom_types, edge_types, alpha, hex_bg_color, num_atom_types
):
    x = positions[:, 0]
    y = positions[:, 1]
    z = positions[:, 2]
...
def generatePIL3d(mol, buffer, bg="white", alpha=1.0):
