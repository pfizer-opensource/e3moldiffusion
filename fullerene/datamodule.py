...
from fullerene.utils import make_splits, MissingEnergyException


class FullereneDataModule(LightningDataModule):
    def __init__(self, hparams, dataset=None):
        super(FullereneDataModule, self).__init__()
    ...
    def setup(self, stage):
        dataset = Fullerene
        self.dataset = dataset(
            root=self.hparams["dataset_root"], **self.hparams["dataset_arg"]
        )
...
if __name__ == "__main__":
    ...
    from tqdm import tqdm
    from fullerene.hparams import add_arguments

    parser = ArgumentParser()
    parser = add_arguments(parser)
    hparams = parser.parse_args()
    print(hparams)
    datamodule = FullereneDataModule(hparams)
