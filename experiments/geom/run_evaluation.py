...
@click.option(
    "--num_graphs", help="Number of graphs to generate", type=int, default=1000
)
@click.option("--batch_size", help="Batch size for generating", type=int, default=100)
@click.option(
    "--device", help="On which device for computation", type=str, default="cuda:0"
)
def main(ckpt_path, save_dir, step, type, server, num_graphs, batch_size, device):
    ...
    if server == "delta":
        ...
        root = (
            "/hpfs/userws/let55/projects/e3moldiffusion/experiments/geom/data"  # alpha
        )
    else:
        raise ValueError
