...
def evaluate(args):
    ...
    for sdf_file in test_files:
        ...
        if not args.filter_by_posebusters and not args.omit_posebusters:
            ...
            buster_combined = pd.concat([buster_mol_df, buster_dock_df], axis=1)
            buster_combined = buster_combined.loc[
                :, ~buster_combined.columns.duplicated()
            ]
            pb_validity = buster_combined.mean(axis=1).mean()
            statistics_dict["pb_validity_per_mol"].append(pb_validity)
