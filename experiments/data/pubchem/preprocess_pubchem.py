...
# def main(files, idx):
# main_path = "/hpfs/userws/cremej01/projects/e3moldiffusion/experiments/pubchem/raw/train_raw"

# files = glob(os.path.join(main_path, "*.gz"))
# files = list(chunks(files, 1000))

# for i in range(len(files)):
#     with open(f"files_{i}.pickle", "wb") as f:
#         pickle.dump(files[i], f)

# pool = Pool(processes=20)
# pool.map(process, files)

# with open(f"data_list_{idx}.pickle", "wb") as f:
#     pickle.dump(data_list, f)
# with open(f"smiles_list_{idx}.pickle", "wb") as f:
...
if __name__ == "__main__":
    ...
    files = f"/hpfs/userws/cremej01/projects/e3mol/files_{args.index}.pickle"
