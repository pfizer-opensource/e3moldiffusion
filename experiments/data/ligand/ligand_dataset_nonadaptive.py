...
class LigandPocketDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        center = True
        if center:
            for i in range(len(mol_data["lig_coords"])):
                mean = (
                    mol_data["lig_coords"][i].sum(0)
                    + mol_data["pocket_coords"][i].sum(0)
                ) / (len(mol_data["lig_coords"][i]) + len(mol_data["pocket_coords"][i]))
                mol_data["lig_coords"][i] = mol_data["lig_coords"][i] - mean
                mol_data["pocket_coords"][i] = mol_data["pocket_coords"][i] - mean
...
class LigandPocketDataModule(AbstractAdaptiveDataModule):
