...
from experiments.data.ligand.priorconfig import CONFIG_5A, CONFIG_8A, CONFIG_10A
...
def _get_bin_idx(space_size, config: dict):
...
def get_prior_distribution_targetdiff(sizes, natoms, boundaries):
  import numpy as np
  import pandas as pd
  boundaries = np.quantile(sizes, q = np.linspace(0.1, 0.9, 9))
  bins = {i:[] for i in range(len(boundaries)+1) }
  ...
  for i in range(len(sizes)):
      ...
      if s < boundaries[0]:
          ...
          idx = np.where(s >= boundaries)[0][-1] + 1
      bins[idx].append(natoms[i])
      
  bins_m = {i:pd.Series(a).value_counts() for i, a in bins.items()}
  bins_m = {i: (a.index.tolist(), (a.values / sum(a.values)).tolist())for i, a in bins_m.items()}
  bins_out = [(a[0], a[1]) for a in bins_m.values()]
  config = {"bounds": boundaries.tolist(), "bins": bins_out}
  return config