...
class Trainer(pl.LightningModule):
    ...
    def sample_chain(self, n_tries, context=None):
        ...
        if (
            ...
            self.hparams["dataset"] == "aqm"
            or self.hparams["dataset"] == "uspto"
            or self.hparams["dataset"] == "spice"
        ):
            n_nodes = 50
        else:
            raise ValueError()
    ...
    def sample_sweep_conditional(
        ...
        if use_openbabel:
            ...
            while valid_samples < n_samples:
                ...
                one_hot, x, n_molecules, failed_ids = sanitize_molecules_openbabel(
                    one_hot, x, batch, self.dataset_info
                )
                contexts.append(
                    torch.cat(
    ...
    def analyze_and_save(
        ...
        stability_dict, rdkit_tuple, midi_log = self.val_sampling_metrics(
            molecule_list, dataset_info=self.dataset_info
        )
    ...
    def sample_different_sizes_and_save(
        ...
        for counter in range(int(n_samples / batch_size)):
            ...
            print(
                f"Generated molecule: Positions {x[:, :]}, Shape {(x.shape[0], x.shape[1])}"
            )
            vis.save_xyz_file(
                path,
                one_hot,
