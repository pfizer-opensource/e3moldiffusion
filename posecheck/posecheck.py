...
from posecheck.utils.chem import rmsd_calc
...
class PoseCheck(object):
    ...
    def load_ligands_from_sdf(self, sdf_path: str, add_hs: bool = False) -> None:
        """Load a ligand from an SDF file."""
        self.ligands = load_mols_from_sdf(sdf_path, add_hs=add_hs)

        if not add_hs:
            self.ligands_nohs = load_mols_from_sdf(sdf_path, add_hs=add_hs)
        else:
            self.ligands = load_mols_from_sdf(sdf_path, add_hs=add_hs)
